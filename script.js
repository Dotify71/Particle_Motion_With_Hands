...
function initThree() {
    ...
    window.addEventListener('resize', () => {
        ...
        renderer.setSize(innerWidth, innerHeight);
        if (bgCanvas) { bgCanvas.width = innerWidth; bgCanvas.height = innerHeight; drawBgScene(); }
    });
}
...
function drawBgScene() {
    ...
    if (currentBg === 'dark') { bgCanvas.style.display = 'none'; return; }
...
const VOICE_COMMANDS = {
    ...
    actions: {
        ...
        'save':            () => takeScreenshot(),
        'more particles':  () => changeParticleCount(1),
        'less particles':  () => changeParticleCount(-1),
    }
};
...
function applyPreset(el) {
    ...
    document.querySelectorAll('.preset-dot').forEach(d => d.classList.remove('active'));
    el.classList.add('active');
    if (color === 'rainbow') { applyColorRainbow(); return; }
    applyColor(color);
}
...
function applyCustomCount() {
    ...
    DENSITY_LEVELS.forEach((l, i) => { const d = Math.abs(l-val); if (d < minDiff) { minDiff=d; nearest=i; } });
...
function initEvents() {
    ...
    window.addEventListener('keydown', e => {
        ...
        const map = {
            'h':'heart','f':'flower','s':'saturn','e':'fireworks',
            'd':'dna','t':'torus','g':'galaxy','y':'pyramid',
            'b':'sphere','w':'wave','m':'mobius','c':'cube',
